...
In the current version of the Editor, it is possible to edit Main Customer Portal view and Login page view. You can also edit views created by Workflows.
...
A custom banner that can contain text, media, and applicable discount codes. Controls allow for changing presets, height, alignment, image placement and CTA URL.
...
When you use [Workflows](/workflows/workflows-overview) for retention, cancellation saves, dunning, polls, and similar flows, each interactive step has a matching **action block** used in the flow overlay.
