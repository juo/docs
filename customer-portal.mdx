...
Subscribers have the ability to cancel their subscription via the Customer Portal by clicking the **Cancel** button. Depending on your settings and workflows, they may be offered save options first. Cancelled subscriptions can be reactivated when **Reactivate cancelled** is enabled in Customer Portal settings.
...
#### Skip a product in the next order

When product editing is allowed, customers can **Skip in next order** on an individual line. That product is left out of the upcoming renewal only; other products still renew. Whole-order skip and per-product skip are both controlled by the **Skip next order** setting in Customer Portal management options.
#### Change renewal date
...
When **pickup points** are enabled, customers can choose or change a parcel locker / PUDO location instead of a home address. See [Shipping setup → Pickup points](/shipping-setup#pickup-points).
...
Control what subscribers can do under **Settings → Customer Portal**:

- Enable the **block-based Customer Portal** (required for the [Editor](/editor))
- Toggle management actions: cancel, pause, skip next order, change renewal date, reactivate cancelled / failed, change delivery method and address, redeem discount codes, edit delivery frequency (including custom frequency), add any product as a one-time purchase
- Control product-line editing: add, remove, swap, quantity, variant, hide descriptions
- Exclude specific products from the portal, and optionally **auto-exclude new products**

Cancel, pause, and skip are unavailable while a subscription is still under its **minimum required renewals**. Change renewal date may be unavailable for fixed fulfillment-day plans.

Redeem discount codes should only be enabled if you share subscription-eligible codes with customers. See [Discounts](/discounts).

Some subscriptions will not be editable before a minimum number of renewals is reached. If this is the case, there will be a notification informing about how many renewals are still required before editing becomes possible.
