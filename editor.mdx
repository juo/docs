...
Enable the **block-based Customer Portal** under **Settings → Customer Portal** before using the Editor.
...
| **Desktop / mobile toggle**  | Check layout on phone-sized and desktop viewports. On mobile you can reorder blocks independently; use the couple control to make mobile match the desktop order again. |
...
For media banners you can set **Media fill** (Cover, Contain, Original, Fill width, Fill height) and **Media alignment**, with separate values for desktop and mobile when needed.
...
| --------------------- | -------------------------------------------------------------------------------------------------- |
| **Layout type**       | **One column** (classic stacked layout) or **Grid** (flexible canvas). Grid is opt-in; you can switch back anytime. Under Grid, block order is spatial rather than a simple list drag. |
...
4. Management options (what subscribers can do in the portal) are still configured in [General settings](/general-settings)—the Editor controls layout and presentation.
